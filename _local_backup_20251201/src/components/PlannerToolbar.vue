...
const emit = defineEmits<{
  ...
  (e: 'save-state-url'): void;
  (e: 'load-state-url'): void;
  (e: 'copy-link'): void;
}>();
...
<template>
  <div class="p-4 rounded-2xl border shadow-sm bg-white mb-6">
    ...
    <div class="flex flex-wrap gap-3 items-center">
      ...
      <button @click="emit('export-csv')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Exportar CSV</button>
      <button @click="emit('export-txt')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Exportar TXT</button>
      <button @click="emit('export-excel')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Exportar Excel</button>
      <button @click="emit('export-word')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Exportar calendari en Word</button>
      <button @click="emit('export-json')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Exportar JSON</button>
      ...
      <button @click="emit('save-state-url')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Guardar estat a l'URL</button>
      <button @click="emit('load-state-url')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Carregar estat de l'URL</button>
      <button @click="emit('copy-link')" class="px-3 py-2 border rounded-xl shadow-sm hover:bg-gray-50">Copiar enllaç</button>
    </div>
    ...
    <div class="mt-4 flex flex-wrap items-center gap-3">
      <div class="flex flex-wrap gap-2">
        ...
        >
          {{ p.tipus === "FINAL" ? "FINAL" : p.tipus === "REAVALUACIÓ" ? "REAVALUACIÓ" : "PARCIAL" }}
        </button>

        <button
          @click="emit('add-period')"
          class="px-4 py-2 rounded-full border text-sm bg-green-50 hover:bg-green-100"
        >
          Afegir període
        </button>
      </div>
      ...
      <button
        ...
        class="px-4 py-2 rounded-full border text-sm bg-red-50 hover:bg-red-100"
      >
